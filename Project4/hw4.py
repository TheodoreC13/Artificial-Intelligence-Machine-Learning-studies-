...
def linreg_closed_form(train_X, train_Y):
	...
	'''
	Jth component of ith example means each i in x[i][j] is an example, so each row is an example, columns are attributes
	...
	use them to build up equation 1 in the function linreg closed form(...). 
	'''
	# theta = (x^T * x)^-1 * (x^T * y)
	# T means transpose, we use numpy.dot for multiplication, numpy.linalg.inv to inverse
	# I was specifically told to use these for this equation, my testing gave this output the as the exact same as
	# numpy.linalg.lstsq(...)
	# TODO: We originally were prepending in here but changed to prepending in main due to issues with gradient
	# train_X = numpy.insert(train_X,0,1,axis=1) # prepend the array with 1's
	inverse = numpy.linalg.inv(numpy.matmul(numpy.transpose(train_X),train_X,out=None))
	# matmul was used while I was testing it's the same as numpy.dot and actually preferred according to documentation
	xty = numpy.matmul(numpy.transpose(train_X),train_Y)
	Theta = numpy.matmul(inverse,xty)
	return Theta
...
def loss(Theta, train_X, train_Y):
	...
	'''
	rv = None
	# Htheta of Xi = X(i)^T * Theta
	# TODO: We originally were prepending in here but changed to prepending in main due to issues with gradient
	# train_X = numpy.insert(train_X, 0, 1, axis=1)  # prepend the array with 1's
	N = len(train_X)
	Sum = 0
	for i in range(N):
		Htheta = numpy.dot(numpy.transpose(train_X[i]), Theta)
		Sum += numpy.square(Htheta - train_Y[i])
	rv = (1/(2*N)) * Sum
	return rv
...
def linreg_grad_desc(initial_Theta, train_X, train_Y, alpha=0.05, num_iters=500, print_iters=True):
	...
	for k in range(1,num_iters+1):
		...
		# New theta = old theta - alpha * (1/N * (X^T * X * Theta - X^T * Y))
		# cur_grad = (numpy.dot(numpy.transpose(train_X), cur_loss))
		# cur_Theta = cur_Theta - alpha/len(train_X) * cur_grad
		transx = numpy.transpose(train_X)
		lossgradient = (1 / len(train_X)) * (numpy.dot(transx, numpy.dot(train_X, cur_Theta)) - numpy.dot(transx, train_Y))
		cur_Theta = cur_Theta - (alpha * lossgradient)
...
def random_fourier_features(train_X, train_Y, num_fourier_features=100, alpha=0.05, num_iters=500, print_iters=False):
	...
	D,N = train_X.shape
	Omega = numpy.random.random_sample((N,num_fourier_features+1))
	B = numpy.random.random(num_fourier_features+1)
...
if __name__ == '__main__':
	...
	# y = numpy.array([5,11])
	# TODO Prepending inside the functions causes issues once you hit gradient
	data_X2 = numpy.insert(data_X,0,1,axis=1)
	true = numpy.linalg.lstsq(data_X2, data_Y)
	print(true)
	theta = linreg_closed_form(data_X2,data_Y)
	print(theta)
	# vis_linreg_model(data_X,data_Y,theta)
	# Plotting
		# This is where i'd put my 3d plot if I could figure it out
	# Plotting over
	losses = loss(theta,data_X2,data_Y)
	print(losses)
	print(losses*2*len(data_X))
	if len(theta) == 2:
		zeroTheta = numpy.array([[0],[0]])
	else:
		zeroTheta = numpy.array([[0],[0],[0]])
	gradient = linreg_grad_desc(zeroTheta,data_X2,data_Y)
	# print(gradient[-1])
	# 10 million features locked my computer, don't do 10 million
	rf = random_fourier_features(data_X,data_Y,num_fourier_features=10000)
	vis_rff_model(data_X,data_Y,rf[0],rf[1],rf[2])
